...
class ShiftEditor extends Component {
  ...
  render() {
          ...
          var sliderList = this.props.data.sort(function(a, b){
            if( a.start === b.start)
              return a.end - b.end;
            return a.start - b.start;
          }).map(function(d, i){
            if( i === length-1)
              return  <Range key={d.id} onChange={seditor.onShiftChange.bind(seditor, d.id)}  marks={marks} min={5} max={32} defaultValue={[d.start, d.end]} tipFormatter={value => `${d.code} ${value}:00`} />;
            if( d.description.includes("Minor") )
              return <Range key={d.id} style={{background: 'red'}} min={5} max={32} defaultValue={[d.start, d.end]} tipFormatter={value => `${d.code} ${value}:00`} />;
